        ...
        return html`
            ...
            <section class="settings-section">
                ...
                <div class="settings-group">
                    ...
                    <label class="switch-label" title="${t('settingsTranscriptionVadTitle')}">
                        ${t('settingsTranscriptionVad')}
                        <md-switch
                            id="transcription-vad-switch"
                            ?disabled=${live(!this.config.transcription.enabled || this.isDownloadingModel)}
                            ?selected=${live(this.config.transcription.vad ?? true)}
                            @input=${this.updateProp('transcription', 'vad')}></md-switch>
                    </label>
            ...
            switch (key1) {
                ...
                case 'transcription':
                    if (key2 === 'language') {
                        ...
                        if (!(e.target instanceof MdSwitch)) return
                        this.config.transcription.vad = e.target.selected
                    }
                    break
                case 'uiTheme':
                    if (!(e.target instanceof MdFilledSelect)) return
